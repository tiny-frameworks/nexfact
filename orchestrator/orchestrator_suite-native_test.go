...
func TestOrchestratorNativeSuite(t *testing.T) {
	...
	// === providers we test ==
	// the only difference to the full test orchestrator_suite_test.go is
	// instead: providers := []string{NATIVE, CONTAINER} we use providers := []string{NATIVE}
	// === providers we test ==
	providers := []string{NATIVE}
